...
// Core localStorage backend + mobile nav toggle
...
document.addEventListener('click', (e)=>{if(e.target && e.target.id==='navToggle'){const nav=document.querySelector('.nav');if(nav) nav.classList.toggle('open');}if(e.target && e.target.closest('.nav') && e.target.tagName==='A'){const nav=document.querySelector('.nav');if(nav) nav.classList.remove('open');}});
